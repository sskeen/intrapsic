...
def condense(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df.drop(columns=[
        ...
        'sui_re',    ### explicit suicidality regex max: pre-disambiguation
    ...
    # Drop '_pred' suffix from column names
    df = df.rename(columns={col: col.removesuffix('_pred') for col in df.columns if col.endswith('_pred')})
